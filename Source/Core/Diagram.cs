...
namespace Dataweb.NShape
{
	...
	public sealed class Diagram : IEntity, ISecurityDomainObject
	{
		...
		/// <summary>Raised when the diagram location changes.</summary>
			...
			set
			{
				if (size.Width != value)
				{
					if (displayService != null)
						displayService.Invalidate(X, Y, Width, Height);
		...
		/// <summary>
		/// X-position of diagram in pixels.
		/// </summary>
		[CategoryLayout()]
		[Description("The X-position of the diagram.")]
		[RequiredPermission(Permission.Layout)]
		public int X
		{
		...
		/// <summary>
		/// Y-position of diagram in pixels.
		/// </summary>
		[CategoryLayout()]
		[Description("The Y-position of the diagram.")]
		[RequiredPermission(Permission.Layout)]
		public int Y
		{
		...
		public Point Location
		{
			...
			set
			{
				...
				location.X = value.X < 0 ? 0 : value.X;
				location.Y = value.Y < 0 ? 0 : value.Y;
				OnMoved(EventArgs.Empty);
		...
		public NamedImage BackgroundImage
		{
			...
			set
			{
				backImage = value;
				InvalidateDrawCache();
				if (displayService != null) displayService.Invalidate(X, Y, Width, Height);
			}
		}
		...
		/// <summary></summary>
		[CategoryMap()]
		[TypeConverter("Dataweb.NShape.WinFormsUI.MapProviderConverter, Dataweb.NShape.WinFormsUI")]
		[DefaultValue("None")]
		public GMap.NET.MapProviders.GMapProvider MapProvider
		{
			get { return DisplayService.MapProvider; }
			set { DisplayService.MapProvider = value; }
		}

		/// <summary>Количество статических свойств</summary>
		public int StaticPropsCount
		{
			get { return DisplayService.StaticPropsCount; }
			set { DisplayService.StaticPropsCount = value; }
		}

		/// <summary>Количество динамических свойств</summary>
		public int DynamicPropsCount
		{
			get { return DisplayService.DynamicPropsCount; }
			set { DisplayService.DynamicPropsCount = value; }
		}
		...
		public void DrawBackground(Graphics graphics, Rectangle clipRectangle)
		{
			...
			Rectangle bounds = Rectangle.Empty;
			bounds.X = Math.Max(X, clipRectangle.X);//!Math.Max(0, clipRectangle.X);
			bounds.Y = Math.Max(Y, clipRectangle.Y);//!Math.Max(0, clipRectangle.Y);
		...
		internal void OnMoved(EventArgs e)
		{
			//if (displayService != null) displayService.NotifyBoundsChanged();
