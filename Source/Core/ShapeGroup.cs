...
namespace Dataweb.NShape.Advanced
{
	...
	public class ShapeGroup : Shape, IShapeGroup
	{
		...
		public string Kind
		{
			get
			{
				List<ShapeRect> shapeRects = new List<ShapeRect>(Children.Count);
				foreach (Shape child in Children)
					shapeRects.Add(new ShapeRect() { shape = child, rect = child.GetBoundingRectangle(true) });
				for (int i = 0, n = shapeRects.Count - 1; i < n; ++i)
					for (int j = i + 1, m = shapeRects.Count; j < m; ++j)
					{
						if (shapeRects[i].rect.Height <= shapeRects[j].rect.Height
						&& shapeRects[i].rect.Width <= shapeRects[j].rect.Width)
						{
							ShapeRect shapeRect = shapeRects[i];
							shapeRects[i] = shapeRects[j];
							shapeRects[j] = shapeRect;
						}
					}
				string kind = string.Empty;
				for (int i = 0, n = shapeRects.Count; i < n; ++i)
					kind += "-" + (shapeRects[i].shape is ShapeGroup ? "Отношение" : shapeRects[i].shape.Type.Name);
				if (kind.Length > 0)
		...
		protected internal new GroupShapeAggregation ChildrenCollection
		{
			...
			internal set
			{
				if (value != null && !(value is GroupShapeAggregation))
					throw new ArgumentException(string.Format("{0}'s ChildrenCollection has to derived from GroupShapeAggregation.", value));
