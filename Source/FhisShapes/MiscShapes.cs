...
namespace Dataweb.NShape.FhisShapes
{
	...
	public class ThickArrow : RectangleBase
	{
		...
		protected override void CalcControlPoints()
		{
			...
			int i = 0;
			ControlPoints[i].X = left;						// 0-ArrowTipControlPoint
			ControlPoints[i].Y = 0;
			++i;
			ControlPoints[i].X = right - BodyWidth;			// 1-ArrowTopControlPoint
			ControlPoints[i].Y = top;
			++i;
			ControlPoints[i].X = right - halfBodyWidth;		// 2-BodyTopControlPoint
			ControlPoints[i].Y = -halfBodyHeight;
			++i;
			ControlPoints[i].X = right;						// 3-BodyEndControlPoint
			ControlPoints[i].Y = 0;
			++i;
			ControlPoints[i].X = right - halfBodyWidth;		// 4-BodyBottomControlPoint
			ControlPoints[i].Y = halfBodyHeight;
			++i;
			ControlPoints[i].X = right - BodyWidth;			// 5-ArrowBottomControlPoint
			ControlPoints[i].Y = bottom;
			++i;
			ControlPoints[i].X = 0;							// 6-MiddleCenterControlPoint
			ControlPoints[i].Y = 0;
		}
